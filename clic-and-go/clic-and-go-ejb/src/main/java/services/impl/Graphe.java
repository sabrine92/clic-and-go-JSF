...
import services.interfaces.grapheLocal;
import services.interfaces.grapheRemote;

/**
 * Session Bean implementation class graphe
 */
@Stateless
public class Graphe implements grapheRemote, grapheLocal {

	private int[][] U;
	private boolean[] valid; // sommet valide veut dire qu'il existe dans le
								// graphe
	...
	public int getValArc(int i, int j) {
		if ((i < 0) || (i >= U.length) || (j < 0) || (j >= U.length)) {
			System.out
					.println("Graphe::getValArc : Erreur: hors de la matrice.");
			System.exit(-1);
		}
		if (!existeArc(i, j)) {
	...
	public int degreEntrant(int i) {
		...
		if ((i < 0) || (i >= U.length)) {
			System.out
					.println("Graphe::degreEntrant : Erreur: hors de la matrice.");
			System.exit(-1);
		}
		if (!existeSommet(i)) {
			System.out
					.println("Graphe::degreEntrant : Erreur: ce sommet n'existe pas.");
			System.exit(-1);
		}
		for (int k = 0; k < U.length; k++) {
	...
	public int degreSortant(int i) {
		...
		if ((i < 0) || (i >= U.length)) {
			System.out
					.println("Graphe::degreSortant : Erreur: hors de la matrice.");
			System.exit(-1);
		}
		if (!existeSommet(i)) {
			System.out
					.println("Graphe::degreSortant : Erreur: ce sommet n'existe pas.");
			System.exit(-1);
		}
		for (int k = 0; k < U.length; k++) {
	...
	public int degre(int i) {
		...
		if (!existeSommet(i)) {
			System.out
					.println("Graphe::degre : Erreur: ce sommet n'existe pas.");
			System.exit(-1);
		}
	...
	public int[] lst_succ(int i) {
		if ((i < 0) || (i >= U.length)) {
			System.out
					.println("Graphe::lst_succ : Erreur: hors de la matrice.");
			System.exit(-1);
		}
		if (!existeSommet(i)) {
			System.out
					.println("Graphe::lst_succ : Erreur: ce sommet n'existe pas.");
			System.exit(-1);
		}
